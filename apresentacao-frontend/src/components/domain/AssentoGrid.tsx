...
const statusCor: Record<StatusAssento, string> = {
  LIVRE: "bg-nevoa/20 hover:bg-nevoa/30",
  PRE_RESERVADO: "bg-azul/60 hover:bg-azul/70",
  OCUPADO: "bg-azul hover:bg-azul-light",
...
export function AssentoGrid({
  ...
  return (
    <div
      className={cn(
        "bg-noite-surface relative space-y-4 rounded-2xl p-6",
        className,
      )}
    >
      {/* Palco */}
      <div className="flex flex-col items-center">
        <div className="bg-azul/40 text-nevoa/60 w-1/2 rounded-t-lg py-2 text-center font-mono text-xs uppercase tracking-[0.4em]">
          Palco
        </div>
        <div className="bg-azul/20 h-1 w-3/4 rounded-full" />
      </div>
      ...
      >
        {fileiras.map(([letra, lista]) => (
          <div key={letra} className="flex items-center gap-2">
            <span className="text-nevoa/40 w-5 shrink-0 text-center font-mono text-xs uppercase">
              {letra}
            </span>
            ...
            <span className="text-nevoa/30 w-5 shrink-0 text-center font-mono text-xs uppercase">
              {letra}
            </span>
          </div>
        ))}
      ...
      {/* Legenda */}
      <div className="border-nevoa/10 flex flex-wrap items-center gap-3 border-t pt-4 text-xs">
        {(Object.keys(statusCor) as StatusAssento[]).map((s) => (
          <span key={s} className="flex items-center gap-1.5">
            <span className={cn("inline-block h-3 w-3 rounded-sm", statusCor[s])} />
            <span className="text-nevoa/60">
              {statusLabel[s]}
              {contagem[s] > 0 && (
                <span className="text-nevoa/90 ml-1 font-mono">
                  {contagem[s]}
                </span>
              )}
            </span>
        ...
        <span className="ml-auto">
          <Badge variant="outline" className="text-nevoa/70 border-nevoa/20">
