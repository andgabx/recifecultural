...
function Logo() {
  ...
  return (
    ...
    >
      <span className="bg-azul text-nevoa flex h-7 w-7 shrink-0 items-center justify-center rounded-md">
      ...
      <motion.span
        ...
        className="font-display text-nevoa whitespace-pre text-sm font-medium"
      >
        Recife Cultural
      </motion.span>
    </Link>
...
function GrupoTitulo({ titulo }: { titulo: string }) {
  ...
  return (
    <motion.p
      ...
      className={cn(
        "text-nevoa/35 overflow-hidden px-3 pb-1 text-[10px] font-semibold uppercase tracking-[0.2em]",
      )}
    >
      {titulo}
    </motion.p>
...
function Rodape({ papel }: { papel: Papel }) {
  ...
  return (
    <motion.div
      ...
      className="border-noite-surface text-nevoa/45 mt-4 overflow-hidden border-t pt-3 text-[11px] leading-snug"
    >
      <span className="text-laranja/80 font-mono uppercase tracking-widest">
        {papel}
      </span>
