...
import recifecultural.dominio.agenda.comentario.ComentarioId;
...
public class PassosDiscutirEventos {
    ...
    private EspectadorId outroEspectadorId;
    private EventoId eventoIdCorrente;
    private ComentarioId comentarioPaiId;

    public PassosDiscutirEventos(ContextoCenario contexto) {
        this.contexto = contexto;
    }

    @Dado("um espectador cadastrado")
    public void umEspectadorCadastrado() {
        contexto.espectador = new Espectador(EspectadorId.gerar(), "Maria Silva", "maria@example.com");
    }
    ...
    public void oEspectadorTentarPostarComentarioComTexto(String texto) {
        try {
            Comentario comentario = new Comentario(ComentarioId.gerar(), contexto.espectador.getId(), eventoIdCorrente, texto);
    ...
    public void oEspectadorTentarPostarComentarioLongo() {
        try {
            ...
            Comentario comentario = new Comentario(ComentarioId.gerar(), contexto.espectador.getId(), eventoIdCorrente, textoLongo);
            contexto.comentarioServico.postar(comentario);
        } catch (Exception e) {
            contexto.excecaoCapturada = e;
        }
    ...
    public void oEspectadorPostarComentario(String texto) {
        contexto.comentario = new Comentario(ComentarioId.gerar(), contexto.espectador.getId(), eventoIdCorrente, texto);
    ...
    public void umComentarioPostadoPeloProprioEspectador() {
        contexto.comentario = new Comentario(
                ComentarioId.gerar(), contexto.espectador.getId(), EventoId.gerar(),
    ...
    public void umComentarioPostadoPorOutroEspectador() {
        outroEspectadorId = EspectadorId.gerar();
        contexto.comentario = new Comentario(
                ComentarioId.gerar(), outroEspectadorId, EventoId.gerar(),
                "Espetáculo muito bonito, valeu a pena cada detalhe!"
        );
        when(contexto.comentarioRepositorio.obter(contexto.comentario.getId()))
                .thenReturn(Optional.of(contexto.comentario));
    ...
    public void umComentarioExistenteNoSistema() {
        contexto.comentario = new Comentario(
                ComentarioId.gerar(), EspectadorId.gerar(), EventoId.gerar(),
    ...
    public void oEspectadorTentarResponderComTexto(String texto) {
        try {
            Comentario resposta = new Comentario(
                    ComentarioId.gerar(), contexto.espectador.getId(), contexto.comentario.getEventoId(), texto, comentarioPaiId
            );
    ...
    public void oEspectadorTentarResponderAComentarioQueNaoExiste() {
        try {
            ComentarioId idInexistente = ComentarioId.gerar();
    ...
    public void osComentariosDoEventoForemListados() {
        EventoId eventoId = contexto.comentario != null
                ? contexto.comentario.getEventoId()
                : (eventoIdCorrente != null ? eventoIdCorrente : EventoId.gerar());
    ...
    public void oEspectadorJaPostouNotaParaOEvento(int valorNota) {
        Comentario jaAvaliou = new Comentario(
                ComentarioId.gerar(), contexto.espectador.getId(), eventoIdCorrente,
